...
openssl req -new -nodes -x509 -newkey rsa:4096 -out localhost.crt -keyout localhost.key -days 365 -subj '/CN=localhost/C=DK/ST=DK/L=Copenhagen/O=Connected Cars' -addext "subjectAltName = DNS:localhost,IP:127.0.0.1"
openssl x509 -in localhost.crt -text -noout

# Create Root ca for client certs
openssl ecparam -name prime256v1 -genkey -noout -out ca.key
openssl req -new -x509 -sha256 -key ca.key -out ca.crt -days 365 -subj '/CN=Test Root CA/C=DK/ST=DK/L=Copenhagen/O=Connected Cars'
